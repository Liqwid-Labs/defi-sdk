...
export const addressToPaymentPubKeyHash = (address: Address): Buffer => {
  ...
  if (!parsedAddress) {
    throw new Error(
      `Cannot get payment pubkey hash from address: address could not be parsed: ${address}`
    );
  }

  const paymentPubKeyHash =
    parsedAddress.payment_cred().to_keyhash() ??
    parsedAddress.payment_cred().to_scripthash();

  if (!paymentPubKeyHash) {
    throw new Error(
      `Cannot get payment pubkey hash from address: no pubkey credential found in address: ${address}`
    );
  }

  return Buffer.from(paymentPubKeyHash.to_bytes());
};
