...
export const addressToPaymentPubKeyHash = (address: Address): Buffer => {
  ...
  const paymentPubKeyHash = parsedAddress.payment_cred().to_keyhash() ?? parsedAddress.payment_cred().to_scripthash();

  if (!paymentPubKeyHash) {
    throw new Error(`Cannot get payment pubkey hash from address: no pubkey credential found in address: ${address}`) ;
  }
