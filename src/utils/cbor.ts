...
export const cborConstructorToObject = <T extends {[k: string]: V}, V>(rawCbor: RawCBOR, decoder: {[k in keyof T]: (c: RawCBOR) => T[k]}): T => {
    if (!Array.isArray(rawCbor)) {
      throw new Error(`Cannot decode cbor constructor: not a constructor: ${rawCbor}`);
    }
