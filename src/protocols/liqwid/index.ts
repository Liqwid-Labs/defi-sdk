...
export const Liqwid: Protocol<LiqwidStateDatum, LiqwidLoanDatum> = {
  ...
  markets: {
    ADA: {
      ...
      stateUtxoAddress:
        "addr1wy8arwz5w2wju2ffp4rlugtnxxd2u0zv3pdf2hwzq0mjwkg77hygh",
      loanTokens: [
        {
    ...
    DJED: {
      ...
      stateUtxoAddress:
        "addr1w93av4qclpq7nyqatmnrfnq8xa7v52l8lv4hjl3lpcxcxdqfjuerl",
      loanTokens: [
        {
      ...
      loanUtxosAddress:
        "addr1w9al35tmf6n6t2a4a4t0wr82s0evvp23srw4mwhgdfzct9getn2u0",
    },
    SHEN: {
      underlyingAsset: {
        policyId: "8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61",
      ...
      stateUtxoAddress:
        "addr1wxy75y6cmq28l6677xr4lwk5f8m9wxww6cgzectqp0cf8fcv6qvgw",
      loanTokens: [
        {
      ...
      loanUtxosAddress:
        "addr1wy6e4crd5ndamykxxapcq0mtuxl36fz8wezumh4q0f00kxcpq94y0",
    },
  },
  parseStateDatum,
  parseLoanDatum,
...
const marketComponentTokenRate =
  ...
  async <M extends ValueOf<(typeof Liqwid)["markets"]>>(
    ...
    const stateDatum = await Query.stateThreadDatum(
      market.stateUtxoAddress,
      market.stateToken,
      Liqwid.parseStateDatum
    );
...
const currentDebtInMarket =
  ...
  async <M extends ValueOf<(typeof Liqwid)["markets"]>>(
    ...
    const pubKeyHashes = isStakeAddress(address)
      ? (await Query.stakeAddressAddresses(address)).map(
    ...
    const loansInMarket = (
      await Promise.all(
        market.loanTokens.map((loanToken) =>
          Query.assetUtxosInAddress(
    ...
    const totalDebt = loansInMarket
      .filter(({ parsedDatum }) =>
        pubKeyHashes.some((pkh) => parsedDatum?.owner.equals(pkh))
      )
